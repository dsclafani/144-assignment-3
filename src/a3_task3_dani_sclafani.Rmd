...
```{r, warning = FALSE, message = FALSE, fig.cap = "Figure 1.0: Chapters of Stephenie Meyer's Twilight are categorized into Beginning, Middle, and End parts by dividing the chapters evenly. The 10 most frequent words written in each part are displayed"}

ggplot(data = part_top10, aes(x = word, y = n)) +geom_col(fill = "cyan4") +
  facet_wrap(~part_reorder)+
  coord_flip()+
  theme_minimal()+
  labs( x = " ", y ="Count", title = "Ten Most Frequent words in Beggining, Middle, and Ending\n of Twilight by Stephenie Meyers")+
  theme(plot.title = element_text(hjust =0.5))
```

*Names of characters were excluded from analysis to allow for accurate analysis of overall tone of text.
...
```
```{r, fig.cap="Figure 2.0: The average sentiment score of each Chapter of Twilight based on Afinn sentiment package."}
mean_afinn_graph
```
...
### NRC Lexicon: More Details about each part sentiments
...
```{r, fig.cap = "Figure 3.0: Average Sentiments for each part of the book based on NRC sentiments. Individual words are categorized into sentiments The top ten sentiments for each part of the book are displayed above"}
